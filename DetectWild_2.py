...
import FeatureWild_2 as F
import TrackWild_2 as T
import numpy as np
import cv2
...
def maskIM(colorIM, threshIM, cnt, x0, y0, x1, y1):
    ...
    #cv2.imshow('colorROI',cv2.resize(colorROI,(640,480)))
    #cv2.waitKey(1000)
...
def debugDisplay(oi, objectArray, match, rectIM):
 # draw bounding box in color representing ID
    objID = int(objectArray[oi, C.TRACK_ID])
    x0 = int(objectArray[oi, C.X0])
    x1 = int(objectArray[oi, C.X1])
    y0 = int(objectArray[oi, C.Y0])
    y1 = int(objectArray[oi, C.Y1])
...
def detectTrackFeature(VID):
    ...
    #stopFrame = 1012 - 742 # set MAX_FRAME in Common_4.py per instructions
    ...
    while(cap.isOpened()):  # start frame capture
        ...
        for objContour in contourList:
            ...
            if area > C.MIN_AREA:
                ...
                if area > C.MIN_AREA and area < C.MAX_AREA and touch == 0:    # only process assigned species objects of good size that don't touch image edge
                    print ('hi from frame:', frameCount, 'oi:', oi)
...
if False:
    #plankton = 'did_1_1'
    vid = r'/users/anthonybravo/Desktop/SFSU/CSC667_opticalEngineering/video/merced/0900m2_6.mp4'
